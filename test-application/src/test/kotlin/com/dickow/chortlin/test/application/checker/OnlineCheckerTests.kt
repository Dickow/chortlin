...
class OnlineCheckerTests {
    ...
    private val cdef = defineCorrelation()
            .add(correlation(onlineFirst.onMethod("method1"), "sid", root())
                    .extendFromInput("sid", root()).done())
            .add(correlation(onlineFirst.onMethod("method2"), "sid", root()).noExtensions())
            .add(correlation(onlineSecond.onMethod("method1"), "sid", root()).noExtensions())
            .add(correlation(onlineSecond.onMethod("method2"), "sid", root()).noExtensions())
            .add(correlation(onlineThird.onMethod("method1"), "sid", root()).noExtensions())
            .add(correlation(onlineThird.onMethod("method2"), "sid", root()).noExtensions())
            .finish()
    ...
    fun `check that online checker works for unrelated choreographies with interleaved execution`() {
        ...
        val cset2 = defineCorrelation()
                .add(correlation(onlineSecond.onMethod("method2"), "sid", root())
                        .extendFromInput("sid", root()).done())
                .add(correlation(onlineThird.onMethod("method1"), "sid", root()).noExtensions())
                .add(correlation(onlineThird.onMethod("method2"), "sid", root()).noExtensions())
                .finish()
