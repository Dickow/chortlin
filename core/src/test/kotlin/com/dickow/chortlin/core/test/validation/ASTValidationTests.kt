...
class ASTValidationTests {
    ...
    fun `check that validation fails for invalid ast end configuration`() {
        val choreography = Choreography.builder()
                .returnFrom(participant(A::class.java).onMethod("receive", A::receive), "error")
                .end()
    ...
    fun `check that validation accepts a valid ast`() {
        val choreography = Choreography.builder()
                .interaction(external, participant(A::class.java).onMethod("receive", A::receive), "valid")
                .end()
    ...
    fun `check that validator fails for paths without end`() {
        val choreography = Choreography.builder()
                .interaction(external, participant(A::class.java).onMethod("receive", A::receive), "invalid")
