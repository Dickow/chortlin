...
class LookupTests {
    ...
    fun `insert and retrieve trigger`() {
        val trigger1 = configurationFactory.createTrigger(LookupTests::endpoint1)
        val trigger2 = configurationFactory.createTrigger(LookupTests::endpoint2)

        lookup.add(trigger1)
        lookup.add(trigger2)

        val retrieved = lookup.lookup(LookupTests::endpoint1)
    ...
    fun `insert and retrieve nested interaction`() {
        val trigger = configurationFactory.createTrigger(
                LookupTests::endpoint1,
                listOf(configurationFactory.createInteraction(LookupTests::endpoint2)))
        lookup.add(trigger)
        val interaction = lookup.lookup(LookupTests::endpoint2)
    ...
    fun `retrieve endpoint that does not exist`() {
        val retrieved = lookup.lookup(LookupTests::endpoint2)
